...
# InfluxDB v3: sensors WITH a unit_of_measurement share a table named by that unit.
# Sensors WITHOUT a unit get their own table named "sensor.<entity_id>" (with dots).
INFLUX_UNIT_TABLE = {
    "daily_energy": "Wh",  # UnitOfEnergy.WATT_HOUR
    "power": "W",  # UnitOfPower.WATT
    # Template sensors (tigo.yaml)
    "tigo_max_daily_energy": "kWh",
    "tigo_max_power": "W",
}

# Suffixes whose sensors have NO unit — each gets its own table
UNITLESS_SUFFIXES = {"readings_today", "tigo_max_readings_today"}


def _entity_id_bare(entity_id: str) -> str:
    """Strip 'sensor.' domain prefix to get the bare entity_id."""
    return entity_id.removeprefix("sensor.")


def _is_unitless(entity_id: str) -> bool:
    """Check if this entity has no unit_of_measurement (own-table mode)."""
    bare = _entity_id_bare(entity_id)
    if bare in UNITLESS_SUFFIXES:
        return True
    for suffix in UNITLESS_SUFFIXES:
        if bare.endswith(f"_{suffix}"):
            return True
    return False


def _table_for_entity(entity_id: str) -> str:
    """Resolve the InfluxDB table for a given HA entity_id."""
    bare = _entity_id_bare(entity_id)
    # Check template sensors first (exact match on bare id)
    if bare in INFLUX_UNIT_TABLE:
        return INFLUX_UNIT_TABLE[bare]
    # Per-panel sensors: match on the metric suffix
    for suffix, table in INFLUX_UNIT_TABLE.items():
        if bare.endswith(f"_{suffix}"):
            return table
...
def sql_latest(entity_id: str) -> str:
    if _is_unitless(entity_id):
        # Unitless sensor: own table — use narrow lookback to avoid parquet file limit
        return (
            f'SELECT value FROM "{entity_id}" '
            f"WHERE time >= now() - interval '6 hours' "
            f"ORDER BY time DESC LIMIT 1"
        )
    table = _table_for_entity(entity_id)
    bare = _entity_id_bare(entity_id)
    return (
        f'SELECT value FROM "{table}" '
        f"WHERE \"entity_id\" = '{bare}' AND $__timeFilter(time) "
        f"ORDER BY time DESC LIMIT 1"
    )
...
def sql_series(entity_id: str) -> str:
    ...
    bare = _entity_id_bare(entity_id)
    return (
        f'SELECT time, value FROM "{table}" '
        f"WHERE \"entity_id\" = '{bare}' AND $__timeFilter(time) "
        f"ORDER BY time"
    )
...
if __name__ == "__main__":
    ...
    output_path = "/workspaces/pytap/dashboards/grafana/pv_production_grafana.json"
